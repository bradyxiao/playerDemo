...
public final class IjkMediaPlayer extends AbstractMediaPlayer {
    ...
    private void initPlayer(IjkLibLoader libLoader) {
        loadLibrariesOnce(libLoader); //load so library
        initNativeOnce(); // init player(ffmpeg)

        Looper looper;
        if ((looper = Looper.myLooper()) != null) { //看看当前线程是否Looper.prepare()
            mEventHandler = new EventHandler(this, looper);
        } else if ((looper = Looper.getMainLooper()) != null) { //主线程是否Looper.prepare()
            mEventHandler = new EventHandler(this, looper);
        } else {
            mEventHandler = null;
        }
